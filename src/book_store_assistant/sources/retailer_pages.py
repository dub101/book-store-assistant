...
def _build_seeded_direct_query_urls(
    record: SourceBookRecord,
    profile: RetailerProfile,
) -> list[str]:
    direct_urls = _build_direct_query_urls(record, profile)
    if record.source_url is None:
        return direct_urls

    source_url = str(record.source_url)
    if source_url in direct_urls or not _is_allowed_domain(source_url, profile.domains):
        return direct_urls

    return [source_url, *direct_urls]


def _candidate_retailer_profiles(record: SourceBookRecord) -> list[RetailerProfile]:
    if record.source_url is None:
        return list(SUPPORTED_RETAILERS)

    source_url = str(record.source_url)
    matching_profiles = [
        profile
        for profile in SUPPORTED_RETAILERS
        if _is_allowed_domain(source_url, profile.domains)
    ]
    remaining_profiles = [
        profile for profile in SUPPORTED_RETAILERS if profile not in matching_profiles
    ]
    return [*matching_profiles, *remaining_profiles]
    ...
    with httpx.Client(timeout=timeout_seconds, follow_redirects=True) as client:
        ...
        for index, fetch_result in enumerate(fetch_results, start=1):
            ...
            direct_query_urls_by_profile = {
                profile.key: set(_build_seeded_direct_query_urls(record, profile))
                for profile in SUPPORTED_RETAILERS
            }
            candidate_profiles = _candidate_retailer_profiles(record)
            ...
            retailer_record, raw_issue_codes = lookup_exact_page_record(
                ...
                direct_query_urls=_build_seeded_direct_query_urls,
            ...
            source_url = (
                str(retailer_record.source_url)
                if retailer_record is not None and retailer_record.source_url is not None
                else None
            )
