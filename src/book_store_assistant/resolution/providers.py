...
from book_store_assistant.resolution.openai_record_field_selector import OpenAIRecordFieldSelector
...
def build_default_record_field_selector(
    config: AppConfig,
) -> RecordFieldSelector | None:
    if config.ai_provider is AIProvider.OPENAI and config.openai_api_key:
        return OpenAIRecordFieldSelector(
            api_key=config.openai_api_key,
            base_url=config.openai_api_base_url,
            model=config.openai_model,
            timeout_seconds=config.request_timeout_seconds,
