...
def test_app_config_reads_non_secret_settings_from_config_file(
    ...
    assert config.retailer_page_cache_dir == Path("custom/cache/retailer_pages")
    assert config.publisher_page_cache_enabled is False
    assert config.retailer_page_cache_enabled is False
    ...
    assert config.retailer_page_negative_cache_ttl_seconds == 900
...
def test_app_config_uses_project_data_directories(monkeypatch) -> None:
    ...
    assert config.retailer_page_cache_dir == Path("data/cache/retailer_pages")
    assert config.publisher_page_cache_enabled is True
    assert config.retailer_page_cache_enabled is True
    ...
    assert config.retailer_page_negative_cache_ttl_seconds == 21600.0
...
def test_app_config_reads_runtime_overrides_from_environment(monkeypatch) -> None:
    ...
    monkeypatch.setenv("BSA_RETAILER_PAGE_CACHE_ENABLED", "0")
    ...
    monkeypatch.setenv("BSA_RETAILER_PAGE_NEGATIVE_CACHE_TTL_SECONDS", "300")
    ...
    assert config.retailer_page_negative_cache_ttl_seconds == 300
...
def test_app_config_falls_back_when_environment_overrides_are_invalid(monkeypatch) -> None:
    ...
    monkeypatch.setenv("BSA_RETAILER_PAGE_CACHE_ENABLED", "not-a-bool")
    ...
    monkeypatch.setenv("BSA_RETAILER_PAGE_NEGATIVE_CACHE_TTL_SECONDS", "still-not-a-float")
