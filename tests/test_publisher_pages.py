...
class UnexpectedSearcher:
    def search(self, query: str, allowed_domains: tuple[str, ...], limit: int = 3) -> list[str]:
        raise AssertionError("publisher search should not run")
...
def test_augment_fetch_results_with_publisher_pages_uses_seeded_official_url() -> None:
    source_url = "https://www.planetadelibros.com/libro/123"
    fetch_results = [
        FetchResult(
            isbn="9780306406157",
            record=SourceBookRecord(
    ...
    augmented = augment_fetch_results_with_publisher_pages(
        ...
        page_fetcher=StubPageFetcher({source_url: PLANETA_HTML}),
        max_retries=0,
    )
