...
@patch("book_store_assistant.resolution.openai_subject_mapper.httpx.post")
def test_openai_subject_mapper_reads_text_from_responses_output_content(mock_post: Mock) -> None:
    mock_response = Mock()
    mock_response.json.return_value = {
        "output": [
            {
                "content": [
                    {"text": '{"subject":"FICCION","confidence":0.93}'}
                ]
            }
        ]
    }
...
@patch("book_store_assistant.resolution.openai_subject_mapper.httpx.post")
def test_openai_subject_mapper_returns_none_on_http_timeout(mock_post: Mock) -> None:
