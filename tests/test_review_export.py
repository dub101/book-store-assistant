...
def test_review_rows_can_be_written_to_excel(tmp_path: Path) -> None:
    ...
    assert sheet.cell(row=2, column=15).value == "generated_synopsis_rejected"
    assert sheet.cell(row=2, column=16).value == "1"
    assert sheet.cell(row=2, column=17).value == "generated_synopsis_too_short"
    assert sheet.cell(row=2, column=18).value == "MISSING_SYNOPSIS"
    assert (
        "Synopsis came from google_books with language 'en'."
        in sheet.cell(row=2, column=19).value
    )
    ...
    assert sheet.cell(row=2, column=17).alignment.wrap_text is True
